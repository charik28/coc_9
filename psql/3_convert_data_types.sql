...
select distinct loc_n,loc_e, substr(loc_n,3,1) from rend24.rendement_; -- only 199
select distinct loc_n, substr(loc_n,3,1) from rend24.rendement_
where loc_n like '%N%'
; -- skkiped only 10 rows


UPDATE rend24.rendement_
SET
    loc_n =replace(loc_n, 'N','°'), loc_e =replace(loc_e, 'N','°')
WHERE substr(loc_n,3,1) ='N';
--[2025-10-16 09:04:15] 168 rows affected in 8 ms
UPDATE rend24.rendement_
SET
    loc_n =replace(loc_n, '° ','°'), loc_e =replace(loc_e, '°','°')
WHERE substr(loc_n,3,1) ='°';
...
select loc_e
FROM rend24.rendement_ r_
where loc_e in (' ', '');

update rend24.rendement_ r_
...
-- waddo 2024 exc paused here
select distinct loc_e,loc_n from rend24.rendement_;
select distinct loc_e,loc_n from rend24.rendement;

UPDATE rend24.rendement
...
;
select distinct mun_u from rend24.rendement_ r_
--where r_.armes_u in (0,1,2,3,4)
--where r_.armes_u not in (null,'',' ')
-- v2024: 50 row
;
...
-- v2024  [2025-10-16 09:09:19] 58 rows affected in 3 ms
select distinct equip_sens_m from rend24.rendement_ r_;
select distinct equip_sens_m from rend24.rendement r
;
update rend24.rendement_ r_
...
alter table rend24.rendement add column equip_sens_info varchar;
...
select distinct equip_sens_u,left(r_.equip_sens_u,3 )::int4 FROM rend24.rendement_ r_
...
-- psy_compr
select distinct psy_compr from rend24.rendement_ r_
where psy_compr like '%.%'
;
select distinct psy_compr from rend24.rendement r;
...
--- cig_u Cigaretttes
select distinct cig_u from rend24.rendement_ r_;
select distinct cig_u from rend24.rendement r;
...
UPDATE rend24.rendement
 ...
 FROM rend24.rendement_ r_
 WHERE r_.id = rendement.id
   and r_.tab_kg not in ('');
-----------------[2025-10-15 13:38:58] 2,145 rows affected in 19 ms
--[2025-10-16 09:11:29] 6,655 rows affected in 37 ms

;


select distinct tab_kg from rend24.rendement_ r_;
select distinct tab_kg from rend24.rendement r;


--------------------- alc_l

select distinct alc_l from rend24.rendement_ r_;
select distinct alc_l from rend24.rendement r;
...
select distinct carb_l from rend24.rendement_ r_;
select distinct carb_l from rend24.rendement r;
...
select distinct alim_kg from rend24.rendement_ r_;
select distinct alim_kg from rend24.rendement r;
...
select distinct alim_l from rend24.rendement_ r_;
select distinct alim_l from rend24.rendement r;
...
select * from rend24.rendement;
select * from rend25.rendement;

-- db buckupde to wadoo:: C:\Users\User\wrk\rend25-data\dzGeo-rendement\insert-fine_data
